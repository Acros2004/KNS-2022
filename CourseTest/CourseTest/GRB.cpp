...
#define TS(n) Rule::Chain::T(n)
	...
	Greibach greibach(
		...
		Rule(NS('S'), GRB_ERROR_SERIES + 0, // неверна€ структура программы (функции и их содержимое)
			4,		// S-> s{NrE;} | d{N}S | tfi(F){NrE;}S  | tfi(F){NrB;} | tfi(){NrB;}

			//функци€ sourse
			Rule::Chain(7, TS('s'), TS('{'), NS('N'), TS('r'), NS('B'), TS(';'), TS('}')),
		...
		Rule(NS('N'), GRB_ERROR_SERIES + 1, // (операторы программы)
			...
			30,
			//вызов существующей функции без присваивани€ переменной
			...
			Rule::Chain(5, TS('t'), TS('f'), TS('i'), TS(';'), NS('N')),
			Rule::Chain(4, TS('t'), TS('f'), TS('i'), TS(';')),
		...
		Rule(NS('E'), GRB_ERROR_SERIES + 2, // ошибка в выражении (выражение)
		...
		Rule(NS('L'), GRB_ERROR_SERIES + 3, //значении элементов массива
			2,
			Rule::Chain(1, TS('l')),
			Rule::Chain(3, TS('l'), TS(','), NS('L'))
		),

		Rule(NS('B'), GRB_ERROR_SERIES + 4, //операнд в выражении
			4,
			Rule::Chain(1, TS('i')),
			Rule::Chain(1, TS('l')),
			Rule::Chain(4, TS('i'), TS('['), TS('i'), TS(']')),
		...
		Rule(NS('M'), GRB_ERROR_SERIES + 5,	// ошибка в выражении (выражение с операцией)
			8,	// M-> vE | vEM
		...
		Rule(NS('O'), GRB_ERROR_SERIES + 7,
			6,	//оператор в условии входа в блок
			Rule::Chain(1, TS('<')),
			Rule::Chain(1, TS('>')),
			Rule::Chain(1, TS('b')),
			Rule::Chain(1, TS('m')),
		...
		Rule(NS('F'), GRB_ERROR_SERIES + 8,		// ошибка в параметрах функции
			2,	// F-> ti | ti,F
			Rule::Chain(2, TS('t'), TS('i')),
			Rule::Chain(4, TS('t'), TS('i'), TS(','), NS('F'))
		),

		Rule(NS('W'), GRB_ERROR_SERIES + 9,	//(подвыражение или передаваемые параметры)
			4,	// W-> i | l | i,W | l,W
			Rule::Chain(1, TS('i')),
			Rule::Chain(1, TS('l')),
