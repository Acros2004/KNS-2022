...
void code_block_generation(std::ofstream* file, int& i, LT::LexTable& LexTable, IT::IdTable& IdTable)
{
	while (LexTable.table[++i].lexema != LEX_RIGHTBRACE)
	{
			...
			while (LexTable.table[i].lexema != LEX_SEMICOLON)
			{
				...
				else if (LexTable.table[i].lexema == LEX_ID)
				{
					if (IdTable.table[LexTable.table[i].idxTI].idtype == IT::IDTYPE::PF)
					{
						...
						if (strcmp(IdTable.table[LexTable.table[function_id].idxTI].id, "printcurrenttime") != 0 && strcmp(IdTable.table[LexTable.table[function_id].idxTI].id, "printcurrentdate") != 0)
		...
		if (LexTable.table[i].lexema == LEX_CONDITION)
		{
			...
			if (LexTable.table[i].lexema == LEX_LESS_OR_EQUAL)
			{
				*file << "ja condition_end\n";
			}
			if (LexTable.table[i].lexema == LEX_EQUAL_IF)
			{
				*file << "jne condition_end\n";
			}
			if (LexTable.table[i].lexema == LEX_NOT_EQUAL_IF)
			{
				*file << "je condition_end\n";
			}
			i += 2;
		...
		*file << FUNCTIONS_BEGIN;
		...
		for (i; i < LexTable.current_size; i++)
		{
			...
			if (LexTable.table[i].lexema == LEX_SOURSE)
			{
				...
				*file << "inend:" << std::endl;
				*file << "call printerror" << std::endl;
				*file << "invoke ExitProcess, 1" << std::endl;
